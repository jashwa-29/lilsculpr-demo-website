...
const API_BASE = 'https://backend.lilsculpr.com/api/special-course';
  ...
  const WORKSHOP_NAME = 'Koi Fish Frame Art';
  const WORKSHOP_PRICE = 499;
  const WORKSHOP_DATE = '2026-08-08';
  ...
  async function checkSlotAvailability() {
    try {
      const params = new URLSearchParams({
        ...
        batch: 'Koi Fish Frame Art 🐟 ⏰ 2:00 PM - 4:00 PM',
        date: WORKSHOP_DATE
      });
      ...
      if (data.success) {
        ...
        console.log(`${avail} / ${cap} slots remaining`);
  ...
  function openRazorpay(orderData, registrationId) {
    return new Promise((resolve, reject) => {
      const options = {
        ...
        description: 'Koi Fish Frame Art',
  ...
  console.log('✅ Koi Fish Frame Art registration form initialized');
  checkSlotAvailability();
})();
